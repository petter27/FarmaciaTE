...
function get_medicamentos() {
  ...
  if (idCat > 0) {
    ...
    $.ajax({
      url: 'includes/functions/CRUD_salidas.php',
      type: 'POST',
...
function agg_med() {
  ...
  if (med != '' && cant > 0 && validarR(med) == false && validarInv(id_med, cant) == true) {
...
function validarInv(_id, _cant) {
  return parseInt(medicamentos[_id].stock) >= _cant;
}
...
//función que hace el post y agrega salida
function finalizar() {
  if (carrito.length > 0) {
    ...
    $.ajax({
      url: 'includes/functions/CRUD_salidas.php',
      type: "POST",
      data: 'agg_salida=' + JSON.stringify(carrito),
      success: function (res) {
        //alert(res);
        if (res == "EXITO") {
